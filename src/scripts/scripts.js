...
for (let index = 0; index < checkboxCategories.length; index++) {
  ...
  checkboxCategory.addEventListener('change', () => {
    ...
    const checkboxActiveCategory = document.querySelectorAll(
      '.categories-search_checkbox._active',
    );

    if (checkboxActiveCategory.length > 0) {
      selectBtnSearch.classList.add('_categories');
      let searchQuantity = selectBtnSearch.querySelector(
        '.search-page_quantity',
      );
...
iconMenu.addEventListener('click', () => {
  ...
  const bottomHeaderColumns = document.querySelectorAll(
    '.bottom-header_column',
  );
